...
const localDir = `public/models/${spec.repo}`;
mkdirSync(localDir, { recursive: true });

console.log(`Downloading ${spec.repo} (${spec.size}) → ${localDir}`);
console.log('Step 1/2: config + tokenizer files...');
execSync(`hf download ${spec.repo} --local-dir "${localDir}" --exclude "onnx/*"`, { stdio: 'inherit' });

console.log(`Step 2/2: ONNX model files (${spec.size})...`);
const fileArgs = spec.files.map(f => `"${f}"`).join(' ');
execSync(`hf download ${spec.repo} --local-dir "${localDir}" ${fileArgs}`, { stdio: 'inherit' });

console.log(`\nDone! Model at: ${localDir}`);
